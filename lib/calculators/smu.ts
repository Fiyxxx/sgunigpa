...
export const smuConfig: UniversityConfig = {
  name: "SMU",
  gpaScale: 4.0,
  grades: [
    { grade: "A", points: 4.0 },
...
export function calculateSMUGPA(courses: Course[]): CalculatedGPA {
  ...
  for (const course of gradedCourses) {
    const gradeMapping = smuConfig.grades.find((g) => g.grade === course.grade);
    if (gradeMapping) {
  ...
  for (const course of pfCourses) {
    ...
    if (gradeMapping && gradeMapping.points >= smuConfig.passFail.passThreshold) {
      earnedCredits += course.credits;
    }
  }
